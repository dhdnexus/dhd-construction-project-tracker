...
export const ProjectModal: React.FC<ProjectModalProps> = ({
  ...
  const [budgetCap, setBudgetCap] = useState<number | string>(25000000);
  const [projectManager, setProjectManager] = useState('');
  const [handoverDate, setHandoverDate] = useState('2026-12-31');
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await onCreateProject({
        ...
        siteAddress: siteAddress.trim() || 'Lagos, Nigeria',
        location: siteAddress.trim() || 'Lagos, Nigeria',
        budgetCap: typeof budgetCap === 'number' ? budgetCap : parseNairaInput(budgetCap) || 25000000,
        projectManager: projectManager.trim() || 'Site Engineer',
        handoverDate: handoverDate || '2026-12-31',
      });
      onClose();
    } catch (err: any) {
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-[#75777E] mt-1 flex items-center gap-1">
                        <MapPin size={12} />
                        <span>{p.siteAddress || p.location || 'Lagos, Nigeria'}</span>
                      </p>
